...
const cardsSize: number = Number(localStorage["value"]) | 4;
let cardsCount: number = cardsSize ** 2 / 2;
document
  .querySelector("html")
...
let gameSrcArray: string[] = srcArray
  .slice(0, cardsCount)
  .concat(srcArray.slice(0, cardsCount));
function shuffle(gameSrcArray: string[]) {
...
for (let i = 0; i < gameSrcArray.length; i++) {
  const card: Card = new Card(gameSrcArray[i]);
  card.createCard();
  CARDS_ARR.push(card);
}
...
function startTimer() {
  ...
  const interval: ReturnType<typeof setTimeout> = setInterval(handler, 1000);
  handler();
  startGame();
  stopGame();
}
...
function sureMsg() {
  const result: boolean = confirm(
    "Press OK if you are sure you want to stap a game else press CANCEL"
  );
...
function finishGame() {
  const timer: string[] = document.querySelector(".timer").innerHTML.split(":");
  const timeInSec = Number(timer[0]) * 60 + Number(timer[1]);
  const userScore: number =
    (numOfComparisons - numOfWrongComparisons) * 100 - timeInSec * 10;

  alert(
    `Congratulations! You've pass the game in ${
