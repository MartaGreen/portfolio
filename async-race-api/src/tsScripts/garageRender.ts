...
const pagesBtns = () => {
  const btnsCont: HTMLDivElement = document.createElement("div");
...
getGarageData().then((garageData) => {
  ...
  const RACE_FIELD: HTMLElement = document.createElement("div");
  ...
  const len: number = document.querySelectorAll(".raceTrack").length;
  BODY.innerHTML += `
  ...
  nextBtn.addEventListener("click", () => {
    pageNum += 1;
    generatePages((pageNum - 1) * 7, (pageNum) * 7);
    (<HTMLElement>document.querySelector(".page")).setAttribute("value", String(pageNum));
    (<HTMLElement>document.querySelector(".page")).innerHTML = `Page #${pageNum}`
  })
