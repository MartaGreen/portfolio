...
export class Car {
  ...
  renderCar(): string {
    const CAR_HTML: string = `<?xml version="1.0" encoding="iso-8859-1"?>
  ...
  move() {
    const race: Element = this.getRace();
    const car: HTMLElement = this.getRendredCar();

    getCarEngine(this.id)
      .then((data) => {
        const startBtn: HTMLElement = race.querySelector(".start");
        const stopBtn: HTMLElement = race.querySelector(".stop");
  ...
  stopMove() {
    getStopCar(this.id).then(() => {
      if (this.carAnim) this.carAnim.cancel();
  ...
  removeCar() {
    const race: Element = this.getRace();
    const removeBtn: HTMLButtonElement = race.querySelector(".remove");
  ...
  updateCar() {
    ...
    const selectBtn: HTMLButtonElement = race.querySelector(".select");

    const carID = this.id;
    const theCar = this;

    selectBtn.addEventListener("click", () => {
      const createCarField: HTMLElement = document.querySelector(".createCar");
      const updateCarField: HTMLElement = document.querySelector(".updateCar");
      createCarField.classList.add("notActive");
      updateCarField.classList.remove("notActive");

      const createBtn: HTMLButtonElement = <HTMLButtonElement>(
        document.querySelector(".create")
      );
      createBtn.setAttribute("disabled", "disabled");

      const updateInput: HTMLInputElement =
        updateCarField.querySelector(".carName");
      const updateColor: HTMLInputElement =
        updateCarField.querySelector(".carColor");

      updateInput.focus();
      updateInput.value = this.name;
      updateColor.value = this.color;

      (<HTMLInputElement>document.querySelector(".createCarName")).value = "";
      (<HTMLInputElement>document.querySelector(".createCarColor")).value =
        "black";
      ...
      updateBtn.addEventListener("click", function addUpdateEvent() {
        ...
        getUpdateCar(carID, newName, newColor).then((carData) => {
          const car: HTMLElement = race.querySelector(".car");
          car.style.fill = carData.color;
          const carTitle: HTMLElement = race.querySelector(".carTitle");
