import { CategoryName, CategoryCard, createAudio } from "../scripts/cards";
...
import { getAllCategories, getAllCategoryCards } from "./requests";
...
const createAttemptsSounds = () => {
  const correctAttempt: HTMLAudioElement = createAttemptSound(
    "correctAttempt",
    "./sounds/correctAttemptSound.mp3"
  );
  const failAttempt: HTMLAudioElement = createAttemptSound(
    "failAttempt",
    "./sounds/failAttemptSound.mp3"
  );
...
const createFinish = () => {
  ...
  const correctFinish: HTMLAudioElement = createAudio(
    "./sounds/correctFinishSound.mp3",
    ["correctFinish"]
  );
  const failFinish: HTMLAudioElement = createAudio(
    "./sounds/failFinishSound.mp3",
    ["failFinish"]
  );
...
function createRegisterBtn() {
  const registerBtn: HTMLInputElement = createBtn("registerBtn", "register");

  registerBtn.addEventListener("click", createRegisterForm);
...
const createMenu = () => {
  ...
  getAllCategories().then((categoriesData) => {
    categoriesData.forEach((category) => {
      createMenuNavItem(category.name.split("_").join(" "), navMenu, "navMenuItem");
    });
  });
  menu.appendChild(navMenu);

  const registerBtn: HTMLInputElement = createRegisterBtn();
...
const pageMain = () => {
  ...
  getAllCategories().then((categoriesData) => {
    ...
    categoriesData.forEach((categoryData) => {
      ...
      const category: CategoryName = new CategoryName(
        categoryData.imgSrc,
        categoryName
      );
      const categoryImg: HTMLDivElement = category.render();

      CATEGORIES_NAMES.push(category);
      CATEGORY_CARDS[categoryName] = [];
      getAllCategoryCards(categoryName).then((cardsData) => {
        cardsData.forEach((cardData) => {
          const createdCard = new CategoryCard(
            cardData.imgSrc,
            cardData.cardName,
            cardData.translate,
            cardData.soundSrc
          );
          CATEGORY_CARDS[categoryName].push(createdCard);
        });
      });

      mainPage.appendChild(categoryImg);
      category.loadCategoryCards();
    });
  });
